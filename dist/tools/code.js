...
function runQuiet(cmd, cwd, timeout = 30) {
    try {
...
function getDiagnostics(file_path, language) {
    ...
    if (lang === "python") {
        ...
        if (rc1 === 0)
            results.push("py_compile: syntax OK");
        else
            results.push(`py_compile:\n${out1.slice(0, 3000)}`);
        const [rc2, out2] = runQuiet(["python", "-m", "flake8", p]);
        if (rc2 === 0 && !out2)
            results.push("flake8: no issues");
        else if (rc2 !== -1)
            results.push(`flake8:\n${out2.slice(0, 3000)}`);
    }
        ...
        if (rc1 === 0 && !out1)
            results.push("eslint: no issues");
        else if (rc1 !== -1)
            results.push(`eslint:\n${out1.slice(0, 3000)}`);
        const [rc2, out2] = runQuiet(["npx", "tsc", "--noEmit", "--strict", p]);
        if (rc2 === 0 && !out2)
            results.push("tsc: no errors");
        else if (rc2 !== -1)
            results.push(`tsc:\n${out2.slice(0, 3000)}`);
    }
...
export function registerCodeTools() {
    registerTool({
        ...
        func: (p) => getDiagnostics(String(p.file_path), p.language),
        read_only: true,
