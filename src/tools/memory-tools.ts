...
export function registerMemoryTools() {
  registerTool({
    ...
    func: (p) => {
      const entry: memory.MemoryEntry = {
        name: String(p.name),
        description: String(p.description),
        type: (p.type as string) || "user",
        content: String(p.content),
        created: new Date().toISOString().split("T")[0],
        scope: (p.scope === "project" ? "project" : "user") as "user" | "project",
        file_path: "",
      };
      memory.saveMemory(entry, (p.scope === "project" ? "project" : "user") as "user" | "project");
  ...
  registerTool({
    ...
    func: (p) => {
      memory.deleteMemory(String(p.name), (p.scope === "project" ? "project" : "user") as "user" | "project");
  ...
  registerTool({
    ...
    func: async (p) => {
      const results = await memory.searchMemory(String(p.query), (p.scope === "project" ? "project" : p.scope === "user" ? "user" : "all") as "user" | "project" | "all");
      if (!results.length) return "No memories found.";
      return results.map((r: memory.MemoryEntry) => `[${r.type}|${r.scope}] ${r.name}: ${r.description}\n${r.content.slice(0, 300)}`).join("\n\n");
    },
    read_only: true,
  ...
  registerTool({
    ...
    func: async (p) => {
      const entries = await memory.loadEntries((p.scope === "project" ? "project" : p.scope === "user" ? "user" : "all") as "user" | "project" | "all");
      if (!entries.length) return "No memories stored.";
      return entries.map((e: memory.MemoryEntry) => `[${e.type}|${e.scope}] ${e.name}: ${e.description}`).join("\n");
    },
    read_only: true,
    concurrent_safe: true,
  });
