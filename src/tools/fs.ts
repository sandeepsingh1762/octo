...
async function readFile(file_path: string, limit?: number, offset?: number) {
  try {
    const p = path.resolve(file_path);
    const stat = await fs.stat(p).catch(() => null);
    if (!stat) return `Error: file not found: ${file_path}`;
    if (stat.isDirectory()) return `Error: ${file_path} is a directory`;
    const content = await fs.readFile(p, "utf-8");
    const lines = content.split("\n");
    const start = offset ?? 0;
    const chunk = limit ? lines.slice(start, start + limit) : lines.slice(start);
    if (!chunk.length) return "(empty file)";
...
async function writeFile(file_path: string, content: string) {
  try {
    const p = path.resolve(file_path);
    ...
    if (!exists) return `Created ${file_path} (${lc} lines)`;
    const diff = unifiedDiff(oldContent, content, path.basename(p));
    if (!diff) return `No changes in ${file_path}`;
...
async function editFile(file_path: string, old_string: string, new_string: string, replace_all = false) {
  try {
    const p = path.resolve(file_path);
    const content = await fs.readFile(p, "utf-8");
    ...
    if (count === 0) return "Error: old_string not found in file. Please ensure EXACT match.";
    if (count > 1 && !replace_all) return `Error: old_string appears ${count} times. Use replace_all=true or provide more context.`;
...
export function registerFsTools() {
  registerTool({
    ...
    func: (p) => readFile(String(p.file_path), p.limit as number | undefined, p.offset as number | undefined),
    read_only: true,
  ...
  registerTool({
    ...
    func: (p) => globFiles(String(p.pattern), p.path as string | undefined),
    read_only: true,
    concurrent_safe: true,
  });
}
