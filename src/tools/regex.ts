...
function regexExtract(pattern: string, text: string, flags?: string) {
  try {
...
function regexReplace(pattern: string, replacement: string, text: string, flags?: string) {
  try {
    const re = new RegExp(pattern, flags || "g");
...
export function registerRegexTools() {
  registerTool({
    ...
    func: (p) => regexExtract(String(p.pattern), String(p.text), p.flags as string | undefined),
    read_only: true,
  ...
  registerTool({
    ...
    func: (p) => regexReplace(String(p.pattern), String(p.replacement), String(p.text), p.flags as string | undefined),
    read_only: false,
    concurrent_safe: true,
  });
}
