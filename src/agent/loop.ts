...
function checkPermission(tc: ToolCall, config: Config): { permitted: boolean; description?: string } {
  ...
  try {
    if (!isModelConfigured(config.model)) {
      yield {
        type: "error",
        message:
          "No model configured. Run /login then /model <provider/model> (e.g. /model openrouter/free).",
      };
      return;
    }
    ...
    while (!state.cancelled) {
      ...
      for (const tc of turnToolCalls) {
        ...
        if (!perm.permitted) {
          const resp: PermissionResponse = { granted: false };
